...
public class URLFactory {
	...
	public URLFactory(String url) {
		...
		if (this.url.indexOf("://") != -1) {
			...
			this.url = this.url.substring(this.url.indexOf("://") + 3, this.url
					.length());
		} else {
			protocol = "http://";
		}
		...
		if (portBeginsAt != -1) {
			if (absolutePathBeginsAt != -1) {
				port = this.url.substring(portBeginsAt + 1,
						absolutePathBeginsAt);
			} else {
		...
		if (absolutePathBeginsAt != -1) {
			absPath = this.url.substring(absolutePathBeginsAt, this.url
					.length());
		} else {
			absPath = "/";
		}
	...
	public String getHttpTcpCellularUrl(String apn, String apnUser,
			String apnPassword) {
		...
		if (isSecured()) {
			return "https://" + host + ":" + port + absPath
					+ ";deviceside=true" + apn + apnUser + apnPassword;
		} else {
			return "http://" + host + ":" + port + absPath + ";deviceside=true"
	...
	public String getHttpWap2Url(ServiceRecord wap2ServiceRecord) {
		if (isSecured()) {
			...
			return "http://" + host + ":" + port + absPath + ";deviceside=true"
					+ ";ConnectionUID=" + wap2ServiceRecord.getUid();
		}
	}
	...
	public String getHttpDefaultTcpCellularUrl(ServiceRecord tcpServiceRecord) {
		if (isSecured()) {
			...
			return "http://" + host + ":" + port + absPath + ";deviceside=true"
					+ ";ConnectionUID=" + tcpServiceRecord.getUid();
		}
	}
	...
	public String getHttpWap1Url(String gatewayAPN, String gatewayIP,
			String gatewayPort, String sourceIP, String sourcePort,
			String username, String password, boolean enableWTLS) {
		boolean paramsValid = true;
		String result;

		if (isSecured()) {
			result = "https://" + host + ":" + port + absPath
					+ ";deviceside=true";
		} else {
			result = "http://" + host + ":" + port + absPath
					+ ";deviceside=true";
		}
	...
	public String getSocketTcpCellularUrl(String apn, String apnUser,
			String apnPassword) {
		if (apn != null && apn.length() > 0)
			apn = ";apn=" + apn;
		else
		...
		if (isSecured()) {
			return "ssl://" + host + ":" + port + ";deviceside=true" + apn
					+ apnUser + apnPassword;
		} else {
			return "socket://" + host + ":" + port + ";deviceside=true" + apn
					+ apnUser + apnPassword;
		}
	}
	...
	public String getSocketDefaultTcpCellularUrl(ServiceRecord tcpServiceRecord) {
		if (isSecured()) {
			return "ssl://" + host + ":" + port + ";deviceside=true"
					+ ";ConnectionUID=" + tcpServiceRecord.getUid();
		} else {
			return "socket://" + host + ":" + port + ";deviceside=true"
					+ ";ConnectionUID=" + tcpServiceRecord.getUid();
		}
	}
	...
	public String getSocketWap1Url(String gatewayAPN, String gatewayIP,
			String gatewayPort, String sourceIP, String sourcePort,
			String username, String password, boolean enableWTLS) {
		boolean paramsValid = true;
		String result;
